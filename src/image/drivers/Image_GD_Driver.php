...
class Image_GD_Driver extends Image_Driver
{
    ...
    function makethumb($properties) {
        ...
        $virtual_image = imagecreatetruecolor($properties['width'],$properties['height']);

        /* copy source image at a resized size */
        if ($status = imagecopyresampled($virtual_image, $this->tmp_image, 0, 0, 0, 0, $properties['width'], $properties['height'], $width, $height)) {
            // Swap the new image for the old one
            imagedestroy($this->tmp_image);
            $this->tmp_image = $virtual_image;
        }
